...
const SalesReturn = () => {
  ...
  const { subtotal, tax, grandTotal } = useMemo(() => {
    ...
    const tax = subtotal * 0.05; // 5% tax adjustment
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        alert('✅ Sales Return / Credit Note saved successfully!');
        // refresh recent list
        const listRes = await apiFetch(`${API}/sales/returns`);
      ...
      alert('Server error while saving return');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageShell title="Create Sales Return (Credit Note)">
            <div className="max-w-6xl mx-auto">
                <form onSubmit={handleSubmit} className="bg-slate-50 p-8 rounded-xl border border-slate-200 shadow-sm space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div>
                            <label htmlFor="creditNoteNo" className="block text-sm font-medium text-slate-700">Credit Note No.</label>
                            <input
                type="text"
                id="creditNoteNo"
                ...
                className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-slate-300 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md">
                        ...
                        <div>
                            <label htmlFor="originalInvoice" className="block text-sm font-medium text-slate-700">Original Invoice No.</label>
                            <input
                type="text"
                id="originalInvoice"
                ...
                required />
                    ...
                    {/* Items Table */}
                    <div className="mt-6">
                         <h4 className="text-lg font-semibold text-slate-800 mb-2">Returned Items</h4>
                            ...
                            <table className="min-w-full divide-y divide-slate-200 border border-slate-200 rounded-lg">
                                <thead className="bg-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider w-32">Return Qty.</th>
                  ...
                  <tr key={index}>
                    ...
                    <div className="flex justify-end">
                        <div className="w-full md:w-1/3 space-y-3">
                            ...
                            <div className="flex justify-between">
                                <span className="font-medium text-slate-600">Tax Adjustment (5%):</span>
                                <span className="font-semibold text-slate-800">Rs. {tax.toFixed(2)}</span>
                            </div>
                             <div className="flex justify-between border-t pt-3">
                                <span className="text-lg font-bold text-slate-800">Total Credit:</span>
                ...
                <div className="mt-6 bg-white border rounded-md p-4">
                  <h4 className="text-sm font-semibold mb-2">Recent Sales Returns</h4>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                      <thead className="bg-slate-50">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Date</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Credit Note No.</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Customer</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Original Invoice</th>
                          ...
                          <th className="px-4 py-2 text-right text-xs font-medium text-slate-500 uppercase">Total Credit</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-slate-200">
                        {recent.length === 0 ? (
                          <tr><td colSpan="7" className="text-center py-4 text-slate-500">No sales returns yet</td></tr>
                        ) : recent.map((r) => (
                          <tr key={r._id}>
                            <td className="px-4 py-2 text-sm">{new Date(r.returnDate).toLocaleDateString()}</td>
                            <td className="px-4 py-2 text-sm">{r.creditNoteNo || '-'}</td>
                            <td className="px-4 py-2 text-sm">{r.customer?.name || '-'}</td>
                            <td className="px-4 py-2 text-sm">{r.originalInvoiceNo || '-'}</td>
