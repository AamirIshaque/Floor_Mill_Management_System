...
const SalesOrder = () => {
  ...
  const [items, setItems] = useState([{ productId: '', quantity: 10, rate: 2500 }]);
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        ...
        } else { setError(cs.message || 'Failed to load customers'); }
        if (pRes.ok) {
          setProducts(ps);
          if (ps.length > 0) setItems([{ productId: ps[0]._id, quantity: 10, rate: 2500 }]);
        } else { setError(ps.message || 'Failed to load products'); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError('');
        // refresh recent list
        const listRes = await apiFetch(`${API}/sales/orders`);
  ...
  return (
    <PageShell title="Create Sales Order">
            <div className="max-w-6xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="bg-slate-50 p-8 rounded-xl border border-slate-200 shadow-sm space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label htmlFor="orderNo" className="block text-sm font-medium text-slate-700">Order No.</label>
                            <input
                type="text"
                id="orderNo"
                ...
                <div className="mt-6 bg-white border rounded-md p-4">
                  <h4 className="text-sm font-semibold mb-2">Recent Sales Orders</h4>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {recent.length === 0 ? (
                          <tr><td colSpan="6" className="text-center py-4 text-slate-500">No sales orders yet</td></tr>
                        ) : recent.map((r) => (
                          <tr key={r._id}>
