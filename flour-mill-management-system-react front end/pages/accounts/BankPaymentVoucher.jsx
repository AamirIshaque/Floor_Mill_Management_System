...
const BankPaymentVoucher = () => {
      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          ...
          if (bankAccounts.length > 0) {
            setPaidFrom(bankAccounts[0]._id);
          }
          // Find expense/liability accounts
          const expenseAccounts = data.filter(acc => acc.type === 'Expense' || acc.type === 'Liability');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const voucherData = {
        ...
        lines: [
          {
            account: paidFrom, // Debit bank account
            narration: `Bank payment to ${payee}${referenceNo ? ` (Ref: ${referenceNo})` : ''}`,
            debit: numAmount,
            credit: 0
          },
          {
            account: expenseHead, // Credit expense/liability account
            narration: narration,
            debit: 0,
            credit: numAmount
          }
      ...
      if (res.ok) {
        setSuccess('Bank Payment Voucher created successfully!');
        // Reset form
        setPayee('');
        setAmount('');
        setReferenceNo('');
  ...
  const expenseLiabilityAccounts = accounts.filter(acc => 
    acc.type === 'Expense' || acc.type === 'Liability');

  return (
    <PageShell title="Bank Payment Voucher">
      {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              ...
              {expenseLiabilityAccounts.map(account => (
                <option key={account._id} value={account._id}>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
              placeholder="A brief description of the bank payment"
              required
            />
          </div>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    <tr>
                      ...
                      <td className="px-4 py-2 text-sm">
                        Bank payment to {payee}{referenceNo ? ` (Ref: ${referenceNo})` : ''}
                      </td>
