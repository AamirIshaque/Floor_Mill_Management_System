...
const GovPurchaseBook = () => {
  ...
  const load = async () => {
    ...
    try {
      setLoading(true);
      const res = await apiFetch(`${API}/gov-wheat/reports/purchase-book?${q.toString()}`);
  ...
  return (
    <PageShell title="Government Purchase Book Report">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {rows.length === 0 ? (
                              <tr><td colSpan="10" className="px-4 py-6 text-center text-slate-500">No data</td></tr>
                            ) : rows.map((r)=> (
                              <tr key={r._id}>
...
export default GovPurchaseBook;